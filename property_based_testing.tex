...
\begin{frame}[fragile]
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw (0,0 -| template.west) node (funct)[right] {\alert{apply("Hi
        @name@",[\{"name","Samuel"\}])}};
      ...
      \draw [->] (templ) -- (template);
      \draw [->] (subst) -- (subs);
      \draw [<->,red] (funct) -| (result) node [midway,above,pos=0.25] {==};
    \end{tikzpicture}
  \end{center}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw<1> (0,5) node (symbol){template()};
      \draw<2> (0,6) node (gen){template()};
      \draw<2>[red] (0,5) node (symbol){[\{text,"Hi "\},\{var,"name","Samuel"\}]};
      \draw (-4,2) node (template) {"Hi @name@"};
      \draw (0,2) node (subs) {[\{"name","Samuel"\}]};
      \draw (4,2) node (result) {"Hi Samuel"};

      \draw (0,0 -| template.west) node (funct)[right] {\alert<1>{apply("Hi
          @name@",[\{"name","Samuel"\}])}};
      ...
      \draw<1> [<->,red] (funct) -| (result) node [midway,above,pos=0.25] {==};
      \draw<2-> [<->] (funct) -| (result) node [midway,above,pos=0.25] {==};
    \end{tikzpicture}
  \end{center}
\end{frame}
